...
### 全文內容刪減約1頁 + v15誕生（2026-06-20續8）

- 使用者要求「在保障語意跟保障重點的條件下」刪除約全文1/8（約1頁）的內容，中英文都要。
- 找出全文中**確實重複**的內容下手（非單純縮句）：
  - §1 Introduction段15+16合併：刪除跟§2.3重複的「Lu et al./CBAM前例」說明、跟§3.2重複的「Sobel固定參數不依賴場景分佈」措辭。
  - §2 Related Work全面緊縮（§2.1/2.2/2.3/2.4各段合併/緊縮描述性詞句，citation與邏輯關係全部保留）。
  - §4.6移除跟§5.2限制分析重複的「699張/7類評估集相對COCO規模偏小」句子；移除跟段166重複的「10/40(25%)」數字重述。
  - §5.2五點限制合併為三點（原1+3條件類合併、原4+5數據限制類合併），未來工作描述精簡。
  - Conclusion移除跟Abstract重複的鋪陳句。
  - Fig.4/Fig.6圖說精簡。
  - 刪除合併後的空段（英文段16/27/190/191，中文對應段16/27/196/197）。
- **實測刪減量**：英文 5067→4526字（刪541字，≈10.7%）；中文 15192→14217字元（刪975字元，≈6.4%）。
- **頁數結果（誠實報告）**：英文/中文PDF重新轉出後**頁數仍維持9頁／8頁，未實際少一頁**。原因：References採單欄排版且本身已無多餘行距可壓縮，目前刪減量只讓原本溢到最後一頁的reference條目往前移1條（從[22]起始移到[23]起始），最後一頁仍有9條reference＋大量空白。若要讓頁數真的從9頁變8頁，估算還需再刪約400+字（總計逼近全文25%），已超出「保障語意與重點」的安全範圍，故未繼續往這個方向加碼刪減。
- **視覺驗證**：兩份PDF重新轉出，逐頁確認所有改寫段落正確渲染、無破版、無遺漏citation。
- **待辦**：
  1. ~~是否接受目前約10%實質刪減量~~ ✅ 使用者要求再刪約1/4頁，見下方新章節
  2. ~~v15尚未推送GitHub~~ ✅ 已完成（commit c7bcf07）
  3. v11~v16六個版本並存，需使用者最終決定哪個是投稿版本

### 第二輪刪減約1/4頁 + v16誕生（2026-06-20續9）

- 使用者要求基於v15再評估、再刪約1/4頁，並先給before/after確認過才套用。
- 找到的最大一塊：**§3.1架構段落**完整重述了一次Fig.1圖說已經逐字描述過的pipeline（Ir→SGA→x'→U-Net→T̂、PatchGAN判別器角色），只保留§3.1獨有的「為何SGA放在最前端」設計理由，其餘交給圖說。
- 其餘5處：§2.2刪除跟§3重複的Pix2Pix架構預告；§4.1.2兩段合併為一段（評估集描述+「無法算PSNR/SSIM/LPIPS」原因合併，因為兩者都源自缺乏ground truth這同一件事）；§3.4 PatchGAN、§4.3 lead-in句、§4.3 perception-distortion句各精簡一點點。
- **實測刪減量**：英文 4526→4444字（再刪82字）；中文 14217→14019字元（再刪198字元）。
- **累計刪減量（兩輪合計）**：英文 5067→4444字（**共刪623字，≈12.3%**，非常接近使用者最初要求的1/8=12.5%）；中文 15192→14019字元（共刪1173字元，≈7.7%）。
- **頁數**：仍維持9頁／8頁不變（原因同前——References單欄區段已無壓縮空間，刪減量反映為頁尾留白增加，不是頁數減少）。
- **視覺驗證**：兩份PDF重新轉出，§3.1（最大改動）、§2.2、§4.3 lead-in/perception-distortion等處逐一截圖確認正確渲染、無破版、無遺漏citation。
- **待辦**：v16尚未推送GitHub；v11~v16六個版本並存，需使用者最終決定投稿版本。
