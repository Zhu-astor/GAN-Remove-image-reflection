...
| Bibkey | 驗證結果 | 是否有建議修正 |
|---|---|---|
| [2] IBCLN | ❌ ERROR（SIR² 歸屬錯誤） | ✅ 已套用（2026-06-13，lines 263-264） |
| [GAP-E] SIR² | ✅ CONFIRMED | 無 |
| [3] Encoder-Decoder | ⚠️ PARTIAL（過度引申） | ✅ 已套用（2026-06-13，lines 265-267, 348） |
| [9] SIRR Survey | (a)✅ / (b)❌ 引用邏輯錯置 | ✅ 已套用（2026-06-13，line 692，已移除【9】） |
| [14] CycleGAN | ✅ CONFIRMED | 無 |
| [23] SMA-Net | (a)✅ / (b)❌ ERROR（跨場景主張不實） | ✅ 已套用（2026-06-13，lines 312-316） |
| [1] CEILNet | ✅ CONFIRMED | 無 |
| [4] Location-aware SIRR | ⚠️ PARTIAL（"空間注意力"措辭） | ✅ 已套用（2026-06-13，lines 267-269） |
...
- **5 個項目的「建議修正」提案，已於 2026-06-13 經使用者核准並套用至 `cvgip2025_chinese.py`**（依 CLAUDE.md §5.2 流程：先提案 → 使用者核准 → 套用）：
  1. **[2]**：第 263-264 行 SIR² 資料集歸屬錯誤 → 已改為「具密集標註 ground truth 的真實場景配對資料集」（移除「SIR²」字樣）
  2. **[3]**：第 265-267, 348 行「深入分析...不可逆」措辭過度引申 → 已改為保守措辭（「下採樣操作所帶來的資訊損失會增加解碼器復原難度」）
  3. **[9]**：第 692 行「自身限制聲明」誤掛外部引用 → 已移除【9】標記
  4. **[23]**：第 312-316 行「驗證了跨場景穩定性」主張與原文（單一 domain、無跨資料集測試）不符 → 已改寫為「啟發本文延伸至跨場景驗證，[23] 本身未測試跨場景，本文跨場景驗證屬新貢獻」
  5. **[4]**：第 267-269 行「空間注意力」措辭與原文「reflection detection module」技術名詞不完全一致 → 已改為「顯式空間位置線索」
  - 套用後已執行 `cvgip2025_chinese.py` 重新生成 `cvgip2025_SGA_chinese.docx`，腳本執行無誤。
